...
CREATE VIEW patient_regimens (regimen_concept_id, patient_id, encounter_id, dispensed_date) AS
  SELECT regimen_ingredients.regimen_concept_id as regiment_concept_id,
         regimen_ingredients.patient_id as patient_id,
         regimen_ingredients.encounter_id as encounter_id, 
         regimen_ingredients.dispensed_date as dispensed_date        
  FROM regimen_ingredients
  GROUP BY regimen_ingredients.encounter_id, regimen_ingredients.regimen_concept_id
  HAVING count(*) = (SELECT count(*) FROM drug_ingredient WHERE drug_ingredient.concept_id = regimen_ingredients.regimen_concept_id); 


-- Count all of the patients whose first 450 regimen disensation happened in the specified period
DROP VIEW IF EXISTS first_line_regimen_dispensations;
CREATE VIEW first_line_regimen_dispensations (patient_id, encounter_id, dispensed_date) AS
...
-- SELECT count(*) AS "Number of patients started on treatment this quarter" FROM (  
-- SELECT patient_id 
-- FROM first_line_regimen_dispensations
-- GROUP BY patient_id
-- HAVING (MIN(DATE(dispensed_date)) >= '2008-04-01' AND MIN(DATE(dispensed_date)) <= '2008-06-30')) as cohort_patients;


-- SELECT patient_id 
-- FROM regimens
-- GROUP BY patient_id
-- HAVING (MIN(DATE(dispensed_date)) >= '2008-04-01' AND MIN(DATE(dispensed_date)) <= '2008-06-30');