...
(function($) {
    function Plot(target_, data_, options_) {
        ...
        function selectPoint(selectedSeries, selectedIndex) {
          drawSeriesPoints(series[selectedSeries], selectedIndex);
          triggerSelectPointEvent(selectedSeries, selectedIndex)
        }  

        function drawSeriesPoints(series, selected) {
            function plotPoints(data, radius, fill, selected) {
                for (var i = 0; i < data.length; ++i) {
                    if (data[i] == null)
                        continue;
                    ...
                    if (i == selected) {
                        ctx.fillStyle = series.points.selectedColor != null ? series.points.selectedColor : series.color;
                        ctx.fill();
                    } else if (fill) {
            ...
            ctx.strokeStyle = series.color;
            ctx.fillStyle = series.points.fillColor != null ? series.points.fillColor : series.color;
            plotPoints(series.data, series.points.radius, series.points.fill, selected);
            ctx.restore();
        }
        ...
        function onClick(e) {
            ...
            pos.pageY = e.pageY;
            pos.hozScale = hozScale;
            pos.vertScale = vertScale;
        ...
        function triggerSelectPointEvent(selectedSeries, selectedIndex) {
          d = series[selectedSeries].data[selectedIndex];
          target.trigger("selectpoint", [{ selectedSeries: selectedSeries,                                           
                                           selectedIndex: selectedIndex, 
                                           x: d[0],
                                           y: d[1],
                                           pageX: Math.round((d[0] - xaxis.min) * hozScale),
                                           pageY: Math.round((yaxis.max - d[1]) * vertScale)}]);
        }
