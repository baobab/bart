class TableVisit 
  ...
  def self.art_visits(patient_id = nil)
    attr_accessor :date, :weight, :height, :anemia, :apptdate, :temp, :side_eff, :treatment_change, :total_pills_left,:arv_supply,:cpt_time_period
    ...
    if side_eff
      side_eff.each do |eff|
        ...
        guardian_present = "Yes" if eff.GuardianAtAppt == 1
        art_continuation = "Stop" if eff.ARVContinuation == 2
        ...
        if eff.ARVSideFX  == 1
          side_effects << "Anemia" if eff.Anemia == -1
          ...
          side_effects << "Rash" if eff.Rash == -1
