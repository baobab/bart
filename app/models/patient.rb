...
class Patient < OpenMRS
  ...
  has_many :observations, :foreign_key => :patient_id do
    ...
    def find_by_concept_name(concept_name)
      find(:all, :conditions => ["voided = 0 and concept_id = ?", Concept.find_by_name(concept_name).id],:order => "obs_datetime ASC") rescue []
    end
    ...
    def find_last_by_concept_name(concept_name)
      find(:first, :conditions => ["voided = 0 and concept_id = ?", Concept.find_by_name(concept_name).id], :order => "obs_datetime DESC") rescue nil
    end
  ...
  def last_encounter_name_by_flow(date = Date.today)
    ...
    self.encounters.find_by_date(date).each do |encounter|
  ...
  def art_guardian(return_guardian_type = false)
    RelationshipType.find(:all).each do |rel_type|
  ...
  def art_guardian_of
    self.people.collect do |people|
      people.related_from.collect do |p|
        p.person.patient.name unless p.attributes["voided"] == true
      end
    end.flatten.compact rescue []
  end
  ...
  def age_in_months(reference_date = Time.now)
    ...
    if reference_date.nil?
      reference_date = self.observations.find(:first, :conditions => ['voided = 0'], :order => 'obs_datetime').obs_datetime rescue Time.now
    end
  ...
  def get_identifier(identifier)
    identifier_list = self.patient_identifiers.collect do |patient_identifier|
      patient_identifier.identifier if patient_identifier.type.name == identifier
    end.compact
  ...
  def art_initial_staging_conditions
    staging_observations = self.encounters.find_by_type_name("HIV Staging").collect do |e|
      e.observations unless e.voided?
    end.flatten.compact rescue nil
    staging_observations.collect do |obs|
      obs.concept.to_short_s if obs.value_coded == Concept.find_by_name("Yes").id
  ...
  def who_stage
    ...
    # the highest stages
    4.downto(2) do |stage_number|
      Concept.find_by_name("WHO stage #{stage_number} #{adult_or_peds}").concepts.each do |concept|
        break if calculated_stage > 1 # stop if we have found one already
        staging_observations.each do |observation|
  ...
  def date_last_art_prescription_is_finished(from_date = Date.today)
    ...
    last_10_give_drugs_encounters.each do |drug_encounter|
    ...
    last_art_encounter.orders.each do |order|
  ...
  def prescriptions(date = Date.today)
    ...
    art_visit_encounters(date).each do |followup_encounter|
  ...
  def art_quantities_including_amount_remaining_after_previous_visit(from_date)
    ...
    quantity_by_drug.each do |drug, quantity|
  ...
  def art_amount_remaining_if_adherent(from_date,use_visit_date=true,previous_art_date=nil)
    ...
    art_quantities_including_amount_remaining_after_previous_visit(date).each do |drug, quantity|
  ...
  def return_dates_by_drug(from_date)
    ...
    art_quantities_including_amount_remaining_after_previous_visit(from_date).each do |drug, quantity|
  ...
  def num_days_overdue_by_drug(from_date)
    ...
    return_dates_by_drug.each do |drug,date|
      num_days_overdue_by_drug[drug] = (from_date - date).floor
    end unless return_dates_by_drug.nil?
  ...
  def recommended_appointment_date(from_date = Date.today,cal_next_appointment_date=true)
    ...
    while(true)
      ...
      ["Saturday","Sunday"].each do |day_to_skip|
        next if Date::DAYNAMES[recommended_appointment_date.wday] == day_to_skip
      end

      # String looks like "1-1,25-12"
      holiday = false
      day_month_when_clinic_closed.split(/, */).each do |date|
        (day,month)=date.split("-")
        holiday = true if recommended_appointment_date.month.to_s == month && recommended_appointment_date.day.to_s == day
        break if holiday
      end
      next if holiday
  ...
  def Patient.find_by_name(name)
    ...
    PatientName.find(:all,
                     :conditions => ["given_name LIKE ? OR family_name LIKE ?","%#{name}%", "%#{name}%"]
                    ).collect do |patient_name|
                    ...
                    PatientIdentifier.find(:all,
                                           :conditions => ["identifier_type = ? AND identifier LIKE ?",name_type_id,"%#{name}%"]).collect do |patient_identifier|
  ...
  def Patient.find_by_age(estimate,birth_year)
    return [] if estimate.blank? || birth_year == 0
  ...
  def Patient.find_by_national_id(number)
    ...
    PatientIdentifier.find(:all, :conditions => ["identifier_type =? AND identifier LIKE ?",
                                                 national_id_type, "%#{number}%"]).collect do |patient_identifier|
      patient_identifier.patient
    end
  end
  ...
  def Patient.find_by_patient_name(first_name,last_name)
    ...
    # collect thos patient name objects into an array of patient objects
    PatientName.find(:all,
                     :conditions => ["given_name LIKE ? and family_name LIKE ?","#{first_name}%", "#{last_name}%"]).collect do |patient_name|
      patient_name.patient
                     end.each do |patient|
                       patient_hash[patient.patient_id] = patient
                     end
  ...
  def Patient.find_by_patient_names(first_name,other_name,last_name)
    ...
    patient_hash = Hash.new
    PatientName.find(:all,:conditions => ["given_name LIKE ? and family_name LIKE ?","#{first_name}%", "#{last_name}%"]).collect do |patient_name| patient_name.patient
    end.each do  |patient|
      patient_hash[patient.patient_id] = patient
    end
    ...
    name_type_id = PatientIdentifierType.find_by_name("Other name").patient_identifier_type_id
    PatientIdentifier.find(:all,
                           :conditions => ["identifier_type = ? AND identifier LIKE ?", name_type_id, "#{other_name}%"]
                          ).collect do |patient_identifier|
                            patient_identifier.patient
                          end.each do |patient|
                            patient_hash[patient.patient_id] = patient
  ...
  def Patient.return_visits(patient_type,start_date,end_date)
    ...
    report = Hash.new
    return patients.collect do |pat|
      next unless pat.art_patient?
      pat_obs = Observation.find(:first,
                                 :include => 'patient',
                                 :conditions => ["DATE(obs.obs_datetime) >= ? AND DATE(obs.obs_datetime) <= ? AND obs.patient_id = ?",
                                                 start_date, end_date, pat.patient_id],
                                 :order => "obs.obs_datetime",
  ...
  def Patient.find_patients_adults(patient_type,start_date,end_date)
    ...
    return patients.collect do |patient|
  ...
  def Patient.virtual_register
    ...
    Observation.find(:all, :group=>"obs.patient_id", :order => "obs_datetime desc").collect do |obs|
      ...
      ["Continue treatment at current clinic","Outcome","Date of first positive HIV test","Is at work/school","Date of ART initiation","Is able to walk unaided","ARV regimen","Weight","Peripheral neuropathy","Hepatitis","Skin rash","Lactic acidosis","Lipodystrophy","Anaemia","Whole tablets remaining and brought to clinic"].each do |concept_name|
        patient_observations = Observation.find(:all,
                                                :conditions => ["concept_id=? and patient_id=?",(Concept.find_by_name(concept_name).id),pat.patient_id],
                                                :order=>"obs.obs_datetime desc").compact
        case concept_name
  ...
  def transfer_out_label(date = Date.today,destination="Unknown")
    ...
    drug_names = ""
    count = 1
    current_art_drugs.each do |name|
  ...
  def self.addup_prescride_drugs(prescriptions)
    ...
    prescriptions.each do |prescription|
      ...
      if prescribe_drugs[drug_name].nil?
        prescribe_drugs[drug_name] = {"Morning" => 0, "Noon" => 0, "Evening" => 0, "Amount" => drug_quantity}
      end
      prescribe_drugs[drug_name].keys.each do |time|
  ...
  def first_cd4_count
    Observation.find(:first,:conditions => ["concept_id=? AND patient_id=?",
                                            (Concept.find_by_name("CD4 Count").id), 
                                            self.patient_id],
                     :order=>"obs.obs_datetime asc")
  end

  def first_cd4_count_date
    Observation.find(:first,:conditions => ["concept_id=? AND patient_id=?",
                                            (Concept.find_by_name("CD4 test date").id),
                                            self.patient_id],
                     :order=>"obs.obs_datetime asc")
  end

  def hl7_arv_number
    self.patient_identifiers.find_by_identifier_type(PatientIdentifierType.find_by_name("Arv national id").id) rescue nil
  end
