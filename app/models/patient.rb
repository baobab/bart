...
=end
  ...
  def Patient.return_visits(patient_type,start_date,end_date)
    ...
    return patients.collect do |pat|
      ...
      pat_obs = Observation.find(:first,
                                 :include=>'patient',
                                 :conditions => ["Date(obs.obs_datetime) >= ? and Date(obs.obs_datetime) <= ? and obs.patient_id=?",start_date,end_date,pat.patient_id],
  ...
  def update_pmtct
    ...
    pmtct_obs = pat_id.collect do |pat|
      pat.observations.find_by_concept_name("Referred by PMTCT").collect do |obs|
        obs.value_coded = "3"
      end
    end
    pmtct_obs.save!
...
=end
  ...
  def self.print_dosage(dosing_frequency)
    # TODO: if this method isn't used it should be deleted
    return nil
