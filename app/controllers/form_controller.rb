class FormController < ApplicationController
  ...
  def show
    ...
    if @form.uri == 'art_reception_check_in' and @patient.arv_number.blank?
      @available_arv_numbers = PatientIdentifier.find(:all,
        :conditions=>['identifier_type=? and voided=0',PatientIdentifierType.find_by_name("Arv national id").id],
        :group => 'identifier').collect{|d|d.identifier.sub(/\w\w\w */,"").to_i}.sort.to_json rescue nil
    end

    if @form.uri == 'art_reception_check_in' 
      @record_pre_arv_number = true if Location.current_location.name == "Zomba Central Hospital"
      @available_arv_numbers = PatientIdentifier.find(:all,
        :conditions=>['identifier_type=? and voided=0',PatientIdentifierType.find_by_name("Pre ARV number ID").id],
        :group => 'identifier').collect{|d|d.identifier.sub(/\w\w\w */,"").to_i}.sort.to_json rescue nil
    end
