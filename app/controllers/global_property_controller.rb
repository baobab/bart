class GlobalPropertyController < ApplicationController
  ...
  def update_params_filter
    update_params :default_scaffold_id => "global_property", :default_sort => nil, :default_sort_direction => "asc"
  end
  ...
  def create
    begin
      @global_property = GlobalProperty.new(params[:global_property])
      @successful = @global_property.save
    rescue
  ...
  def edit
    begin
      ...
      @successful = !@global_property.nil?
    rescue
      flash[:error], @successful  = $!.to_s, false
    end
  ...
  def update
    begin
      @global_property = GlobalProperty.find(params[:id])
      @successful = @global_property.update_attributes(params[:global_property])
    rescue
      flash[:error], @successful  = $!.to_s, false
    end
  ...
  def destroy
    begin
      @successful = GlobalProperty.find(params[:id]).destroy
    rescue
      flash[:error], @successful  = $!.to_s, false
    end
