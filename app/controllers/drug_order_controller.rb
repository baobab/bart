class DrugOrderController < ApplicationController
  def dispense
    ...
    @patient.drug_orders_for_date(session[:encounter_datetime]).each do |drug_order|
  ...
  def create
    redirect_to :controller => "patient", :action => "index" and return if params["dispensed"].nil?

    Order.transaction do
      # makes sure that everything saves, if not roll it all back so we don't pollute the db with half saved records
      DrugOrder.transaction do
        encounter = new_encounter_by_name("Give drugs")
        order_type = OrderType.find_by_name("Give drugs")

        params["dispensed"].each do |drug_id, quantity_and_packs|
          ...
          order.save
          1.upto(number_of_packs) do |pack_index|
