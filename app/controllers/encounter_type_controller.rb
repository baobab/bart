class EncounterTypeController < ApplicationController
  ...
  def create
    begin
      ...
      @successful = @encounter_type.save
    rescue
  ...
  def update
    begin
      @encounter_type = EncounterType.find(params[:id])
      @successful = @encounter_type.update_attributes(params[:encounter_type])
    rescue
      flash[:error], @successful  = $!.to_s, false
    end
  ...
  def destroy
    begin
      @successful = EncounterType.find(params[:id]).destroy
    rescue
      flash[:error], @successful  = $!.to_s, false
    end
